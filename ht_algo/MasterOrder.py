...
from SplitOrder import SplitOrder
...
class MasterOrder(object):
    ...
    def aggregate(self) -> pd.DataFrame:
        for split in self.splitOrders:
            new_row = {'Account': split.originalOrder.account.number, 'Symbol': split.ticker,
                       'Type': split.originalOrder.transactionType.value, 'DollarAmount': split.dollarAmount}
    ...
    def allocateAccounts(self, filledMasterOrderFile: pd.DataFrame) -> pd.DataFrame:
        ...
        for index, row in filledMasterOrderFile.iterrows():
            ordersToAllocate = self.masterTable[
                (self.masterTable['Symbol'] == row['Symbol']) & (self.masterTable['Type'] == row['Type'])]
            ...
            for index2, row2 in ordersToAllocate.iterrows():
                ...
                new_row = {'Account': row2['Account'], 'Symbol': row2['Symbol'], 'Type': row2['Type'],
